...
Expect<void> Executor::runStructNewOp(Runtime::StackManager &StackMgr,
  ...
  if (IsDefault) {
    auto *Inst =
        Runtime::HeapManager::newStruct(StackMgr.getModule(), DefIndex, N);
    ...
    auto *Inst = Runtime::HeapManager::newStruct(StackMgr.getModule(), DefIndex,
                                                 std::move(Vals));
    StackMgr.push(RefVariant(Inst->getDefType(), Inst));
  }
  return {};
...
Expect<void> Executor::runArrayNewOp(Runtime::StackManager &StackMgr,
  ...
  if (InitCnt == 0) {
    auto *Inst =
        Runtime::HeapManager::newArray(StackMgr.getModule(), DefIndex, ValCnt);
    StackMgr.push(RefVariant(Inst->getDefType(), Inst));
  } else if (InitCnt == 1) {
    auto *Inst =
        Runtime::HeapManager::newArray(StackMgr.getModule(), DefIndex, ValCnt,
                                       packVal(VType, StackMgr.getTop()));
    StackMgr.getTop().emplace<RefVariant>(Inst->getDefType(), Inst);
  } else {
    auto *Inst = Runtime::HeapManager::newArray(
        StackMgr.getModule(), DefIndex, packVals(VType, StackMgr.pop(ValCnt)));
    StackMgr.push(RefVariant(Inst->getDefType(), Inst));
  }
  return {};
}
...
Executor::runArrayNewDataOp(Runtime::StackManager &StackMgr,
  ...
  auto *Inst = Runtime::HeapManager::newArray(StackMgr.getModule(),
                                              Instr.getTargetIndex(), N);
