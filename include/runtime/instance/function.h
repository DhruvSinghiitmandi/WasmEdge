...
public:
  ...
  FunctionInstance(const ModuleInstance *Mod, const uint32_t TypeIdx,
                   const AST::FunctionType &Type,
                   Span<const std::pair<uint32_t, ValType>> Locs,
                   AST::InstrView Expr) noexcept
      : CompositeBase(Mod, TypeIdx), FuncType(Type),
